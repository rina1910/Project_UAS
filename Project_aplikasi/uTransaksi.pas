  ...
  try
    ...
    begin
      ...
      begin
        Close;
        SQL.Text :=
          'SELECT dt.id, t.customer, t.tanggal as tanggal_transaksi, dt.jumlah, dt.diskon as diskon_beli_barang, ' +
          't.diskon as diskon_transaksi, t.sub_total, t.total, db.harga_beli, db.harga_jual, db.nama as nama_barang, ' +
          'k.nama as kategori FROM detail_transaksi as dt JOIN data_barang as db ON db.id = dt.barang_id ' +
          'JOIN kategori as k ON k.id = db.id JOIN transaksi as t ON dt.transaksi_id = t.id ' +
...
begin
  ...
  begin
    Close;
    SQL.Text :=
      'SELECT dt.id, t.customer, t.tanggal as tanggal_transaksi, dt.jumlah, ' +
      'dt.diskon as diskon_beli_barang, t.diskon as diskon_transaksi, t.sub_total, t.total, ' +
      'db.harga_beli, db.harga_jual, db.nama as nama_barang, k.nama as kategori ' +
      'FROM detail_transaksi as dt ' +
      'JOIN data_barang as db ON db.id = dt.barang_id ' +
      'JOIN kategori as k ON k.id = db.id ' +
      'JOIN transaksi as t ON dt.transaksi_id = t.id';
    Open;
  end;
end;
...
procedure TTfrmTransaksi.DBNavigator1Click(Sender: TObject; Button: TDBNavButtonType);
var
